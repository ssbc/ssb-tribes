...
module.exports = function GetGroupTangle (server, keystore) {
  ...
  function updateCache (msg) {
    ...
    if (msgGroupId in cache) {
      // Add message to Reduce
      if (tangles && tangles.group && tangles.group.previous) {
        cache[msgGroupId].addNodes([{ key: msg.key, previous: tangles.group.previous }]) // Get key and previous from msg
      }
    }
  }
  ...
  return function getGroupTangle (groupId, cb) {
    ...
    if (groupId in cache) {
      return cb(null, {
        root: info.root,
        previous: Object.keys(cache[groupId].state)
      })
    }
    ...
    pull(
      ...
      pull.collect((err, nodes) => {
        ...
        cb(null, { root: info.root, previous: Object.keys(reduce.state) })
      })
    )
  }
}
