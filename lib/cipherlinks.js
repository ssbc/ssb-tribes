...
const encode = require('private-box2/util/slp-encode')
const TYPES = require('box2-spec/encoding/tfk.json')
const isCloakedId = require('./is-cloaked-msg-id')

class Cipherlink {
  constructor ({ type, format, key } = {}) {
    if (key && key.length !== 32) throw new Error(`Cypherlinks expected to have key of length 32 bytes, got ${msg.length}`)
